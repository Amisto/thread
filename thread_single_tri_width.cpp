...
char fname_dummy[100] = "chain_tri_width_%s.csv", fname[100];
int min_intact = N, check_intact_counter = 0, check_intact_checker, fnum = 0;

//Pulse properties:
double pulse_width = 30;                //mm
//double pulse_height = 0.1;
...
void init_chain()
{
    ...
    for (int i = 0; i < 2*hw; i++)
        force[N/2-hw+i].y = pulse_amplitude*(1.0 - fabs(((double)i-hw)/hw));//*cos((-hw+i)*M_PI/hw/2.0)*cos((-hw+i)*M_PI/hw/2.0);
