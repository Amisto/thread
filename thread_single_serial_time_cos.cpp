...
FILE* flog = NULL;
char fname_dummy[100] = "chain_serialN_time_cos_%s.csv", fname[100];
...
void step_chain()
{
    ...
    if (step*DT < pulse_duration)
    {
        ...
        for (int i = 0; i < 2*hw; i++)
            force[N/2-hw+i].y = pulse_amplitude*cos((-hw+i)*M_PI/hw/2.0)*cos((-hw+i)*M_PI/hw/2.0)*cos((step*DT - pulse_duration/2.0)/pulse_duration/2.0*M_PI/2.0);
    }
