...
char fname_dummy[100] = "chain_camel_serial_%s.csv", fname[100];
int min_intact = N, check_intact_counter = 0, check_intact_checker, fnum = 0, max_intact = 0;

//Pulse properties:
double pulse_width = 50;                //mm
double pulse_amplitude = 5E9;    //Pa, base - 5*10^9 Pa
double pulse_gap = 25;
double pulse_diff = 1.0;
...
void init_chain()
{
    ...
    int hg = int(pulse_gap/DX/2.0);
    for (int i = 0; i < 2*hw; i++)
        force[N/2-hw+i-hg].y = pulse_diff*pulse_amplitude*cos((-hw+i)*M_PI/hw/2.0)*cos((-hw+i)*M_PI/hw/2.0);
    for (int i = 0; i < 2*hw; i++)
        force[N/2-hw+i+hg].y = fmax(force[N/2-hw+i+hg].y, pulse_amplitude*cos((-hw+i)*M_PI/hw/2.0)*cos((-hw+i)*M_PI/hw/2.0));
    for (int i = 0; i < N-1; i++)
        intact[i] = 1;
    going = 1;
...
int main(int argc, char** argv)
{
    ...
    for(pulse_diff = 0; pulse_diff < 1.05; pulse_diff += 0.1)
        fprintf(flog[0], "%lf ", pulse_width);
    fputc('\n', flog[0]);
    
    for(pulse_gap = 0; pulse_gap < 50; pulse_gap += 10)
    {
        printf("starting with pulse_gap %lf\n", pulse_gap);
        for(pulse_diff = 0; pulse_diff < 1.05; pulse_diff += 0.1)
        {
            printf("starting with pulse_diff %lf\n", pulse_diff);
            init_chain();
            for (step = 0; ymax < 100.0; step++)
            {
                step_chain();
            }
            fprintf(flog[0], "%lf ", (max_intact - min_intact)*DX);
        }
        fputc('\n', flog[0]);
    }
