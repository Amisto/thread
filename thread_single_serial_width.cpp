...
FILE* flog = NULL;
char fname_dummy[100] = "chain_serialN_width_%s.csv", fname[100];
...
int main(int argc, char** argv)
{
    ...
    for(pulse_width = 10; pulse_width < 155; pulse_width += 10)
        fprintf(flog, "%lf ", pulse_width);
    fputc('\n', flog);
    ...
    for (int i=0; i<10; i++)
    {
        ...
        fprintf(flog, "%d ", N);
        for(pulse_width = 20; pulse_width < 155; pulse_width += 10) 
        {
            printf("starting with width %lf\n", pulse_width);
            init_chain();
