...
char fname_dummy[100] = "chain_cos4_width_%s.csv", fname[100];
...
void init_chain()
{
    ...
    for (int i = 0; i < 2*hw; i++)
        force[N/2-hw+i].y = pulse_amplitude*cos((-hw+i)*M_PI/hw/2.0)*cos((-hw+i)*M_PI/hw/2.0)*cos((-hw+i)*M_PI/hw/2.0)*cos((-hw+i)*M_PI/hw/2.0);
